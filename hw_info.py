...
def main():
    ...
    try:
        # Get GPU VRAM
        smi_out = subprocess.check_output(["powershell.exe", "-c", "nvidia-smi -q -d MEMORY"], text=True)
        match = re.search(r"FB Memory Usage\s*\n\s*Total\s*:\s*(\d+)\s*MiB", smi_out)
        if match:
            info["vram_gb"] = round(int(match.group(1)) / 1024.0, 1)

        name_out = subprocess.check_output(["powershell.exe", "-c", "nvidia-smi -L"], text=True)
        name_match = re.search(r"GPU 0: (.*?)\s*\(UUID", name_out)
        if name_match:
            info["gpu_name"] = name_match.group(1)

        # Get System RAM
        ram_out = subprocess.check_output(["powershell.exe", "-c", "(Get-CimInstance Win32_OperatingSystem).TotalVisibleMemorySize"], text=True)
